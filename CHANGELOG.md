...
### 다음 릴리스
- **LiteLLM 게이트웨이에서 모델 별칭을 그 자리에서 받아 옵니다.** 게이트웨이가 `GET /model/info` 로 이미 알고 있는 프로파일 ID와 모델명의 대응을 그대로 읽어 오므로, 위임 통계가 쌓이기를 기다리거나 `profile-map.json` 을 손으로 편집할 필요가 없어졌습니다. `sprag profile-map` 으로 캐시된 매핑을 확인하고 `sprag profile-map --refresh` 로 갱신합니다. 이 경로가 막힌 게이트웨이에서는 기존 투표 학습이 그대로 동작합니다.
- 이 머신에서 실측한 결과, 위임 18건이 모델 ID 를 읽지 못해 절감 집계에서 빠져 있었습니다. `sprag route-scan` 의 unresolved 안내도 수동 편집 대신 `profile-map --refresh` 를 먼저 권합니다.
