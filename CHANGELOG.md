...
v3.47.0 다음 릴리스부터 이 프로젝트는 **Apache License 2.0** 을 따릅니다. 원문은 [LICENSE](./LICENSE) 에 있고, Apache-2.0 이 요구하는 출처 표기는 [NOTICE](./NOTICE) 에 정리했습니다.

v3.47.0 까지 받으신 사본은 계속 MIT 조건을 따릅니다. 이미 받은 사본의 조건은 이후 변경이 소급되지 않으므로, 쓰고 계신 버전을 그대로 두신다면 조치할 일은 없습니다. 함께 배포하는 fluent-korean 지침 원문은 이 변경과 무관하게 자체 MIT 조건과 LICENSE 파일을 유지합니다.
