...
**앞의 두 줄이 무엇을 말하는가.** 위임 규칙은 세 가지 기준에 기대고 있고, 가장 자연스러운 반박은 저가 모델로 내리면 정확도를 잃는다는 것입니다. 공개 데이터에서는 그렇지 않았습니다. 같은 기준을 라우터로 구현해 11,696건에 적용한 결과가 단일 최고 모델보다 오히려 조금 높았고 비용은 3분의 1이었습니다. 정확도가 올라간 이유가 이 발상의 근거입니다. 어떤 모델도 모든 유형에서 1등이 아니기 때문입니다.

이 평가는 **오프라인**이라 두 줄은 움직이지 않습니다. 벤치마크가 13개 모델의 응답을 인스턴스마다 점수와 토큰 수, 비용까지 미리 수집해 배포하므로, 라우터는 전 인스턴스에 대해 실제로 돌아가 모델을 골랐고 정확도와 비용은 그 선택에 해당하는 값을 조회해 얻었습니다. API 호출은 없었고 비용도 들지 않았습니다. 검증 대상은 판정 기준이며 sprag 가 실제로 쓰는 모델 목록이 아닙니다. 데이터셋의 모델 풀에 Claude 계열이 없어서 deepseek-v3, qwen3-235b, gpt-5 가 각각 저가·중간·상위 티어를 대신합니다.

다음 두 줄은 이 머신의 원장이고 계속 늘어나므로 기준 시점을 밝힙니다. 아래 수치는 2026-09-18 기준입니다.
...
여기 적힌 값은 모두 도구가 스스로 보고하는 수치입니다. 위 블록은 `sprag route-scan savings`, 사용량은 `sprag --days 30`, 문서 변환은 doc2md 원장에서 가져왔습니다. 추정치는 하나도 없습니다.
...
벤치마크가 공개 데이터로 기준을 다퉜다면, 이 절은 같은 질문을 직접 던진 결과입니다. 이 저장소의 실제 작업에서 프롬프트 여덟 개를 골라 상위 등급과 저가 등급에 각각 보내고, 손으로 적은 정답과 대조했습니다(2026-09-17, 원자료 `video/out/measure2.json`).
...
흥미로운 지점은 둘 다 틀린 문항입니다. 첫 문항이 `process.env` 를 참조하는 파일을 묻는데 하위 디렉터리를 포함하는지 적지 않았고, 두 등급이 각각 나름대로 타당한 해석으로 답했습니다. 그 조건을 넣어 다시 물으니 두 답이 정확히 일치했습니다. 모호한 요청은 모델 문제가 아니라는 평범한 교훈입니다. 남은 한 건은 저가 등급이 파일 목록에서 항목을 빠뜨린 경우인데, 틀린 목록은 확인할 수 있는 목록입니다. "실패가 드러나는가" 기준이 겨냥하는 바로 그 성질입니다.

그러므로 저가 등급이 공짜는 아닙니다. 이 표본에서는 조회 작업 여덟 건당 한 건 정도를 값으로 치르고 비용의 4분의 1을 냅니다. 그래서 규칙은 실패가 드러나는 작업만 내려보내고 설계와 진단은 보내지 않습니다.
...
두 규칙의 증가 양상은 다릅니다. 래칫 규칙은 사람이 실제로 저지른 실수에서 나오므로 상한이 없습니다. 반면 위임 규칙은 포화합니다. 티어 판정이 알아보는 작업 유형이 고정된 집합이라, 유형마다 규칙이 하나씩 생기면 더 승격할 것이 남지 않습니다.
