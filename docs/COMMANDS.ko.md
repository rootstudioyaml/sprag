...
| 이미 읽은 경로 | 호출한 세션의 트랜스크립트 꼬리에서 `Read`·`Edit`·`MultiEdit`·`Write`·`NotebookEdit`·`Grep`·`Glob` 가운데 현재 작업 디렉터리 안쪽 경로를 하나라도 찾았을 때 채워집니다. 파일만 올립니다. `Grep`·`Glob` 의 인자가 디렉터리면 제외합니다. 읽으라고 건네면 서브에이전트가 상한 한 칸을 실패로 쓰기 때문입니다. 제어 문자가 든 경로도 제외합니다. 이 목록은 프롬프트 안에서 한 줄이 한 경로이기 때문입니다 |
