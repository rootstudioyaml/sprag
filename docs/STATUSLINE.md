...
| `🤖` | Active model |
| `🔬 high` | Effort level the session is running at (`/effort`), in **Claude Code's own colors for those levels**, so the chip and the picker that set it agree: yellow at `low` (`warning`), green at `medium` (`success`), blue at `high` (`permission`), purple at `xhigh` and `ultracode` (`autoAccept` / `effortUltra`), and the rainbow spread across the word at `max`. Two of those are animated in the app — `xhigh` shimmers and `max` cycles — and a statusline cannot animate (it re-runs on a seconds-long interval, each run a fresh process), so both render as their still form. That leaves `xhigh` and `ultracode` sharing a hue, which is the app's own doing; the level word tells them apart. Colors follow your Claude Code `theme` setting, including the daltonized themes, and fall back to its ANSI themes on terminals without 24-bit color. Absent on models that carry no effort setting, and on Claude Code older than 2.1.276. `/effort ultracode` is xhigh plus dynamic workflow orchestration and reaches the statusline as plain `xhigh`, so the chip recovers the difference from the session transcript: the `/effort` command's own output, written the moment the command returns, and the ultracode system-reminder that lands when the next turn is assembled, whichever is later. So the chip follows the switch immediately, and a session that starts with ultracode already on shows `xhigh` until its first turn |
